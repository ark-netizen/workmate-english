...
Do not put colleague, manager, or client names in the event title, summary, goal, stage, shared_request, or reply guidance. Names belong only in sender identity, natural greetings, and email sign-offs. shared_request in particular must name NO person at all — it is the one sentence each of the three will later rephrase in their own voice, so a name inside it ends up with the client emailing about themselves in the third person, or listing the user's own colleague and manager back at them. Write it as the task the user has to do ("Send the final visuals for 'One Love' by 3 PM today."), never as a narrator's summary of who wants it.
...
The sentence above is an internal note describing the task — it is NOT your wording. Never paste it in as-is. You are one person writing to the user: speak in the first person ("I need…", "Could you…"), never refer to yourself by name in the third person, and never list or speak on behalf of the user's other contacts — you only know your own side of this.
...
- corrections: every self-written sentence that has a genuine issue (drawn ONLY from sentences the user actually composed themselves, never a marked verbatim-copied line). Returning 0 corrections is completely fine when every reply was already natural or every sentence was a copied suggestion. These five rules are absolute — check the list against them before you output:
  C1. FIX ONLY WHAT IS ACTUALLY WRONG — spelling, grammar, or genuinely unnatural phrasing. The following are NOT errors and must never be "corrected", nor mentioned in the note as if they were:
      ...
      · capitalization that is only a formatting convention: "3 pm" vs "3 PM" vs "3 p.m.", "email" vs "e-mail". All of these are correct English — leave whichever the user wrote alone, and never list it as a reason in the note. Capitalization IS a real error in exactly two cases, and only these: the first letter of a sentence, and the pronoun "I" ("hi manager. i'm working on it" → "Hi Manager. I'm working on it" is a genuine fix). Do not let a legitimate sentence-case fix pull an am/pm reformat along with it
...
- register_feedback: an array with ONE entry per relationship (colleague/manager/client) the user actually replied to today — skip a role entirely if the user never replied to it, do not pad with empty entries. Each entry is a compact "conversation replay" card, NOT prose, and must never repeat what workday_summary already says — say something NEW and SPECIFIC to that one exchange:
  ...
  HARD RULE — never point out a mistake without also giving the fix: if a register_feedback note says the user's sentence has a grammar/word-choice error (e.g. "'needed more time' 등의 문법적 오류가 있으며"), that exact sentence MUST also appear as a "before" in corrections with its corrected "after". Telling the user their sentence is wrong here and then leaving it out of corrections means they are told they failed but never shown how to fix it — that is the single worst failure this report can have. Before you finish, walk through EVERY relationship the user replied to and confirm each of their self-written sentences landed in good_expressions or corrections; a relationship whose reply appears in neither list is a bug, not a stylistic choice.
